...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const hostPlayersList = document.getElementById("hostPlayersList"); // Added to show joined players
    ...
    hostBtn.addEventListener("click", () => {
        ...
        db.ref(`games/${gameId}/players`).on("value", snapshot => {
            ...
            Object.keys(players).forEach(name => {
                ...
                hostPlayersList.appendChild(div);
            });
        });

        // Listen for called numbers to sync host display
        db.ref(`games/${gameId}/calledNumbers`).on("value", snapshot => {
            ...
            lastNumberDiv.textContent = nums[nums.length - 1] || "";
        });
